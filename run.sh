...
extract_latest_blockline() {
  grep -Ei 'Last block|block found|Found block|Generated block|Blockhash|Consensus height' \
    "$NODE_LOG" "$POOL_LOG" "$MINER_LOG" 2>/dev/null | tail -n 8 || true
}

extract_pool_activity() {
  grep -Ei 'share|accepted|getwork|worker|solution|blockhash|difficulty|connected' \
    "$POOL_LOG" 2>/dev/null | tail -n 15 || true
}

extract_node_status() {
  grep -Ei 'Consensus height|Last block|Known Peers|Number of Outbound connections|Total number of nodes|MEMPOOL|Local address' \
    "$NODE_LOG" 2>/dev/null | tail -n 15 || true
}

extract_miner_activity() {
  grep -Ei 'searching for solutions|Updated CUDA block hash|Mh/s|MH/s|Gh/s|GH/s|miner|Loading heavy3a.bin' \
    "$MINER_LOG" 2>/dev/null | tail -n 15 || true
}
...
# =========================
# Safe cleanup before start
# Only Bismuth processes and Bismuth screen sessions
# =========================
log_event "Stopping old Bismuth processes and cleaning only Bismuth screen sessions"
